...
class DhikrCounterNotifier extends StateNotifier<DhikrCounterState> {
  ...
  void loadTodayData() {
    ...
    if (data != null) {
      ...
      final hasCustomDhikrs = loadedData.items.any((item) => item.isCustom == true);
      ...
      if (customDhikrsFromPreviousDays.isNotEmpty) {
        ...
        state = state.copyWith(
          todayData: newData.copyWith(items: allItems),
        );
      } else {
  ...
  List<DhikrItem> _getCustomDhikrsFromPreviousDays() {
    ...
    for (int i = 1; i <= 7; i++) {
      ...
      if (previousData != null) {
        try {
          final previousModel = DhikrCounterModel.fromJson(_deepConvert(previousData));
          
          // Find custom dhikrs (isCustom == true)
          for (final item in previousModel.items) {
            if (item.isCustom == true) {
              // Check if we already have this dhikr (by title)
              final alreadyAdded = customDhikrs.any((cd) => cd.title == item.title);
              if (!alreadyAdded) {
